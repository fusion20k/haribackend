...
### [x] Phase 7: Stripe Checkout Trial Flow (extension uses hosted Checkout page)

**Files**: `index.js`

- [x] Add `POST /billing/create-trial-checkout-session` endpoint:
  - Requires auth
  ...
  - Returns `{ checkoutUrl }`
- [x] Update `checkout.session.completed` webhook handler:
